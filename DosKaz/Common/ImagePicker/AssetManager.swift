...
open class AssetManager {
  ...
  public static func getImage(_ name: String) -> UIImage {
    ...
    if let resource = bundle?.resourcePath, let resourceBundle = Bundle(path: resource + "/ImagePicker.bundle") {
      bundle = resourceBundle
		...
		return UIImage(named: name, in: bundle, compatibleWith: traitCollection) ?? UIImage(named: name) ?? UIImage()
  }
  ...
  public static func resolveAssets(
		_ assets: [PHAsset], size: CGSize = CGSize(width: 720, height: 1280)) -> [UIImage] {
    let imageManager = PHImageManager.default()
    let requestOptions = PHImageRequestOptions()
